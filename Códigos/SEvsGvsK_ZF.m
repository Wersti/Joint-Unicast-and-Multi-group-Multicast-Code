...
for n = 1:3
    for G = 1:length(G_set)
        for K = 1:length(K_set)
            for i = 1:iter 
                if N_values(n) > U + G_set(G)
                    resp = Simulation_th2y4([U, ones(1,G_set(G))*K_set(K)],N_values(n), P_un, P_mu);
                    resultados_totales(G, n, K,i) = resp.SE_jk_mu;
                else
                    resultados_totales(G, n, K,i) = 0;
                end
            end
        end
    end
...
%title('SE mínima  vs G vs K para ZF');
